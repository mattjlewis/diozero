...
/**
 * java -cp
 * tinylog-1.2.jar:diozero-core-$DIOZERO_VERSION.jar:diozero-provider-mmap-$DIOZERO_VERSION.jar
 * GpioMmapTest 12 40000000
 */
public class GpioMMapTest {
	...
	private static void setPullUpDown(int gpio, int pud) {
		// pigpio:
		/*
		 * try { gpioReg.put(GPPUD, pud); // Sleep 20us Thread.sleep(0, 20_000);
		 * gpioReg.put(GPPUDCLK0 + gpio >> 5, 1 << (gpio & 0x1F)); Thread.sleep(0,
		 * 20_000); gpioReg.put(GPPUD, 0); gpioReg.put(GPPUDCLK0 + gpio >> 5, 0); }
