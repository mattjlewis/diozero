...
public class SysFsGpioUtil {
	...
	static void export(int gpio, DeviceMode mode) {
		if (!isExported(gpio)) {
	...
	static Path getGpioDirectoryPath(int gpio) {
		return rootPath.resolve(GPIO_DIR_PREFIX + gpio);
	}

	static void unexport(int gpio) {
		if (isExported(gpio)) {
	 ...
	 */
	static public boolean isExported(int gpio) {
