...
public class OdroidN2PlusMmapGpio implements MmapGpioInterface {
    ...
    /*
     * The actual base address of the register is 0xFF634400. However, that is not aligned with the page boundaries. In
     * order to deal with this, we use the previous page boundary (0xFF634000). Since we are dealing with 32-bit ints,
     * we make up the 0x400 byte difference by adding 0x100 to each of the offsets. Therefore, none of these match the
     * documented addresses or offsets, but they all actually work.
     */
    ...
    public synchronized void initialise() {
        if (!initialised) {
            j2MappedByteBuffer = new MmapIntBuffer(MEM_DEVICE, J2_GPIO_BASE_ADDRESS, BLOCK_SIZE, ByteOrder.LITTLE_ENDIAN);
    ...
    public DeviceMode getMode(int gpio) {
        ...
        if (fsel == -1 || shift == -1) {
            Logger.error("Invalid GPIO {}", gpio);
    ...
    public static void main(String[] args) {
        ...
        System.out.format("gpioToGPSETReg(%d)=0x%04x%n", 214,
                          gpioToOutputRegOffset(214));
        System.out.format("gpioToGPSETReg(%d)=0x%04x%n", 219,
                          gpioToOutputRegOffset(219));
        System.out.format("gpioToGPFSELReg(%d)=0x%04x%n", 214,
                          gpioToOutputEnableRegOffset(214));
        System.out.format("gpioToGPFSELReg(%d)=0x%04x%n", 219,
                          gpioToOutputEnableRegOffset(219));
        ...
        try (OdroidC2MmapGpio mmap_gpio = new OdroidC2MmapGpio()) {
            ...
            if (true) {
                ...
                System.out.format("Duration for %,d iterations: %,.3f s, frequency: %,.0f Hz%n",
                                  iterations, ((float) duration_ms) / 1000,
                                  frequency);
            }
